...
def get_market_data():
    ...
    full_df = full_df.sort_values(by=['Sort_Key', 'Ticker']).drop(columns=['Sort_Key'])
    full_df['No.'] = range(1, len(full_df) + 1)

    return full_df, current_rate, update_time, unique_tickers
...
with st.spinner('최신 데이터를 불러오는 중...'):
    df, exchange_rate, last_update, all_ticker_list = get_market_data()

# --- [변경] 텍스트 입력 대신 멀티 선택 박스 적용 ---
selected_tickers = st.sidebar.multiselect(
    "관심 종목 선택 (최대 200개)",
    options=all_ticker_list,
    default=[],
    help="목록에서 종목을 선택하거나 직접 입력하여 검색하세요."
)

view_option = st.sidebar.selectbox("조회 모드", ["전체 종목 보기", "변동성 종목만 보기 (1%↑)"])

# 필터링 로직
# 선택된 종목이 있으면 해당 종목만, 없으면 전체 표시
if selected_tickers:
